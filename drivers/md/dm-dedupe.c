...
static inline bool _is_empty_pg(struct page *page, size_t offset, size_t len)
{
	bool is_zero;
	int i;
...
static bool is_bvec_zero(struct bio_vec *bvec)
{
	...
	if (bvec->bv_len != PAGE_SIZE)
		return false;
	return _is_empty_pg(bvec->bv_page, bvec->bv_offset, bvec->bv_len);
}
...
MODULE_AUTHOR("Shaun Tancheff");
MODULE_DESCRIPTION("dedupe target for performance testing");
MODULE_LICENSE("GPL");

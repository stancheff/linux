...
static void ext4_prealloc_pages_fill_locked(struct inode *inode,
		...
		pgoff_t index, struct inode *inode, unsigned int len)
{
	...
	ext4_prealloc_pages_fill_locked(inode, len, gfp);
	folios = &EXT4_I(inode)->i_free_folios;
	folio = __filemap_get_folio(mapping, index, folios, fgp_flags, gfp);
